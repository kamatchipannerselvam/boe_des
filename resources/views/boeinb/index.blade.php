...
<td><br><span style="text-align:right"><a class="btn btn-labeled btn-success bold" style="font-size:9px; font-weight:bold" href="{{ route('obstockreq.send-order') }}"><span class="btn-label"><i class="glyphicon glyphicon-ok"></i></span>SEND ORDER</a></span></td>
</tr>
</table>
</th>
</tr>
...
<tbody>
    ...
    @foreach($finalarray as $skey=>$stitem)
    <tr>
        <td>{{ $stitem['boe_number'] }}</td>
        <td>{{ $stitem['vendor_name'] }}</td>
        <td>{{ $stitem['category'] }}</td>
        <td>{{ $stitem['brand'] }}</td>
        <td>{{ $stitem['model_no'] }}</td>
        <td>{{ $stitem['model_name'] }}</td>
        <td>{{ $stitem['hscode'] }}</td>
        <td>{{ $stitem['coo_code'] }}</td>
        <td>{{ $stitem['color'] }}</td>
        <td>
        <div id="after_action">
        <span style="font-size:12px" class="inpqty" id="qty_available_{{ $stitem['inbstockinfoid'] }}" data-value="{{ $stitem['inbstockinfoid'] }}">{{ number_format($stitem['qty'],2)}}</span>
        @if($stitem['rqty']>0)
        <span class="btn btn-danger btn-circle btn-sm" style="font-size:10px;" id="qty_Order{{ $stitem['inbstockinfoid'] }}" data-value="{{$stitem['rqty'] }}">{{ $stitem['rqty'] }}</span>
        @else
        <span class="btn btn-danger btn-circle btn-sm" style="font-size:10px; display:none" id="qty_Order{{ $stitem['inbstockinfoid'] }}" data-value="0"></span>
        @endif
        </div>
        </td>
        <td class="w-10">
        @if($stitem['rqty']>0)
        <input type="text" class="form-input w-15" autocomplete="off" id="qty_{{ $stitem['inbstockinfoid'] }}" readonly/>
        @else
        <input type="text" class="form-input w-15" autocomplete="off" id="qty_{{ $stitem['inbstockinfoid'] }}"/>
        @endif                
        </td>
        <td>
        <div class="btn-group">
        ...
        <li><a href="javascript:void(0);" class="btn btn-link" onclick="action_button({{ $stitem['inbstockinfoid'] }}, 'Order')">Order</a></li>
        <li><a href="javascript:void(0);" class="btn btn-link" onclick="action_button({{ $stitem['inbstockinfoid'] }}, 'Delete')">Delete</a></li>
        </ol>
        </div>
        </td>
    </tr>
...
$.post("{{ url('obstockreq/create-order') }}", {"_token": "{{ csrf_token() }}", 'req':'action','qty':qty_order,'stock_id':id_stock,'req_type':req_type},
function(response) {
